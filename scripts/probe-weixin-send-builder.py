...
const builder = mod.base.add(0x15e8200);
const fillReq = mod.base.add(0x33fc800);
const fillReqCaller = mod.base.add(0x15e830d);
...
send({
  ...
  fill_request: fillReq.toString(),
  fill_request_callsite: fillReqCaller.toString(),
});

Interceptor.attach(fillReq, {
  onEnter(args) {
    try {
      if (!this.returnAddress.equals(fillReqCaller)) {
        this.skip = true;
        return;
      }
      this.skip = false;
  ...
  onLeave(retval) {
    try {
      if (this.skip) return;
      const strings = scanStdStrings(this.reqPtr, 0xf0);
      const upstreamStrings = scanStdStrings(this.arg1, 0x60);
      const key = JSON.stringify(strings);
      if (seen[key]) return;
      seen[key] = true;
      if (Date.now() < baselineUntil) return;
      send({
        kind: 'send_builder_event',
        return_address: frameInfo(mod, this.returnAddress),
        request_ptr: safePtrString(this.reqPtr),
        arg1_ptr: safePtrString(this.arg1),
        arg2_value: safePtrString(this.arg2),
        request_strings: strings,
        arg1_strings: upstreamStrings,
      });
    } catch (e) {
      send({ kind: 'error', where: 'fillReq', error: String(e) });
    }
  }
});
