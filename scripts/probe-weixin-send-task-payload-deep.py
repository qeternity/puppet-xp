...
function readUtf8Maybe(ptrValue, maxLen) {
  try {
    if (ptrValue.isNull()) return null;
    const s = ptrValue.readUtf8String(maxLen);
    if (!s) return null;
    return s;
  } catch (e) {
    return null;
...
function readUtf16Maybe(ptrValue, maxChars) {
  try {
    if (ptrValue.isNull()) return null;
    const s = ptrValue.readUtf16String(maxChars);
    if (!s) return null;
    return s;
  } catch (e) {
    return null;
...
function scanPointers(base, sizeBytes) {
  ...
  for (let off = 0; off <= sizeBytes; off += Process.pointerSize) {
    try {
      ...
      const utf8 = readUtf8Maybe(p, 128);
      const utf16 = readUtf16Maybe(p, 128);
      const std = readStdString(p);
      const around = scanStdStrings(p, 0x40);
      if (!plausibleText(utf8) && !plausibleText(utf16) && !plausibleText(std) && around.length === 0) {
        continue;
      }
  ...
  try {
    ...
    while (cur.compare(end) < 0 && count < 8) {
      out.push({
        ...
        pointers: scanPointers(cur, 0x60),
      });
      cur = cur.add(stride);
      count++;
    }
...
const copyPayload = mod.base.add(0x15affc0);
...
send({
  ...
  copy_payload: copyPayload.toString(),
});

Interceptor.attach(copyPayload, {
  onEnter(args) {
    try {
      const src = args[1];
      const payload = {
        dst_ptr: safePtrString(args[0]),
        src_ptr: safePtrString(src),
        flat_strings: scanStdStrings(src, 0xc0),
        flat_pointers: scanPointers(src, 0xc0),
        vector_at_28: scanVector(src.add(0x28)),
      };
      ...
      send({
        kind: 'send_task_payload_deep',
        payload,
      });
    } catch (e) {
      send({ kind: 'error', where: 'copyPayload', error: String(e) });
    }
  }
});
