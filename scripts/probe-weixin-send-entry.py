...
function scanRecipientVector(vecPtr) {
  const out = [];
  try {
    ...
    while (cur.compare(end) < 0 && count < 8) {
      const s = readStdString(cur);
      const ptrStr = safePtrString(cur);
      out.push({
        element_ptr: ptrStr,
        value: s,
      });
...
Interceptor.attach(entry, {
  onEnter(args) {
    try {
      const req = args[0];
      const textGuess = readStdString(req.add(0x20));
      const around = scanStdStrings(req, 0x120);
      const recipients = scanRecipientVector(req.add(0x8));
      const key = JSON.stringify([textGuess, around, recipients]);
      if (seen[key]) return;
      seen[key] = true;
      if (Date.now() < baselineUntil) return;
      send({
        kind: 'send_entry_event',
        req_ptr: safePtrString(req),
        text_guess: textGuess,
        request_strings: around,
        recipients,
      });
    } catch (e) {
