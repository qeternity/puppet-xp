...
function stackFrames(context) {
  return Thread.backtrace(context, Backtracer.ACCURATE).slice(0, 12).map(addr => {
...
Interceptor.attach(parseIfNeeded, {
  onEnter(args) {
    this.msgObj = args[0];
    this.frames = stackFrames(this.context);
    this.returnAddress = this.returnAddress ? this.returnAddress.toString() : null;
  },
  onLeave(retval) {
    try {
      const event = extractMessage(this.msgObj, selfUsername);
      if (!event.conversation_id && !event.content) return;
      const key = eventKey(event);
      if (seen[key]) return;
      seen[key] = true;
      if (Date.now() < baselineUntil) return;
      if (emittedStacks >= 3) return;
      emittedStacks += 1;
      send({
        kind: 'live_message_stack',
        event,
...
setInterval(() => send({ kind: 'heartbeat', seen_count: Object.keys(seen).length, emittedStacks }), 30000);
"""
